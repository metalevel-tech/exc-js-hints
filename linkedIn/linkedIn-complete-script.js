/**
 * DOM to Image script from https://www.npmjs.com/package/dom-to-image
 */
(function (global) {
  "use strict";
...
class Lesson {
  ...
  async setDataAndProcess(courseName, chapters, lessons, downloadType = "all") {
    ...
    const duration = currentLesson
      .querySelector(".classroom-toc-item__title + div")
      .innerText.replace(/ /, "")
      .trim();
    ...
    this.chapterTitle = currentChapter
      .querySelector("h2 span.classroom-toc-section__toggle-title")
      .innerText.replace(/\n.*$/, "")
      .replace(/:/g, " -")
      .replace(/^\d+\s*[-.]\s/, "")
      .trim();
    ...
    const lessonsInCurrentChapter =
      currentLesson.parentElement.querySelectorAll("li");
    this.lessonNumber =
      Array.from(lessonsInCurrentChapter).indexOf(currentLesson) + 1;
    ...
    this.lessonIndex = `[${(lessons.indexOf(currentLesson) + 1)
      .toString()
      .padStart(lessonsCountLength, "0")} - ${lessonsCount}]`;
...
function download(
  ...
  const buttons = document.querySelectorAll(
    "section > h2 > button.classroom-toc-section__toggle"
  );
  ...
  setTimeout(() => {
    ...
    if (listOfLessonsToDownload) {
      ...
      lessonsLoop = lessons.filter((value, index) =>
        listOfLessonsToDownload.includes(index + 1)
      );
      console.log(
        `Chapters: ${chapters.length}, Lessons: ${lessons.length}, Download items: ${listOfLessonsToDownload}`
      );
    } else {
      lessonsLoop = [...lessons];
      ...
      console.log(
        `Chapters: ${chapters.length}, Lessons: ${
          lessons.length
        }, Start from: ${startFromLesson}, Download: ${
          lessonsNumberToDownload ? lessonsNumberToDownload : "all"
        }`
      );
    }
    ...
    async function collectData() {
      ...
      for (const lesson of lessonsLoop) {
        if (
          lessonsNumberToDownload &&
          counter++ >= lessonsNumberToDownload &&
          !listOfLessonsToDownload
        )
          return;

        if (
          lesson !== document.querySelector(".classroom-toc-item--selected") &&
          startFromLesson !== 0
        )
...
// download(9, null, null, "video");
download(1, null, null, "video");
